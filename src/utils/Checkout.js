...
const Checkout = ({ priceId, text, classNames, cancelUrl }) => {
  ...
  useEffect(
    () =>
      setStripe(
        window.Stripe(process.env.NEXT_PUBLIC_STRIPE_PK, {
          stripeAccount: 'acct_1IedUYL4te1d25v7',
        })
      ),
    []
  );
  ...
  return (
    <button onClick={goToCheckout} className={`btn-primary ${classNames}`} type="button">
      {text}
    </button>
  );
};
